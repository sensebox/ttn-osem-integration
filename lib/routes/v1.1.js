...
const router = require('express').Router(),
  ...
  dbModels = require('openSenseMapAPI/lib/models'),
  cfg = require('../../config'),
  ...
  request(`${cfg.osemEndpoint}/boxes`)
    ...
    .then(boxes => {
      ...
      for (const box of boxes) {
        if (box.integrations.ttn.devID === dev_id) {
          if (box.integrations.ttn.appID === app_id) {
      ...
      return Promise.reject(
        `no box found for dev_id '${dev_id}' and app_id '${app_id}'`
      );
    })
    .catch(err => {
